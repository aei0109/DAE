...
if __name__ == '__main__':
    ...
    pca_codes = pca.transform(mnist.test.images) # 차원을 줄이는 함수
    print(pca_codes)
    
    scatter(pca_codes, mnist.test.labels)
    
    pca_recon = pca.inverse_transform(pca_codes[:1]) #inverse_transform을 진행할 데이터 (차원을 다시 복원시키는 함수)
    plt.imshow(pca_recon[0].reshape((28,28)), cmap=plt.cm.gray) #그래프에 띄울 몇번 째 데이터인지
    plt.show()
